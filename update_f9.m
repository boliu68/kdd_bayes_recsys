function para = update_f9(para, hyperpara)
    ...
    for j = 1:d
        for k = 1:h
           %h dimension
            v_mV_n9jk = (1 ./ ((1 ./ para.v_mV(k)) - (1 ./ para.h_v_mV9(j,:))));
            m_mV_n9jk = v_mV_n9jk .* (para.m_mV(k) .* (1 ./ para.v_mV(k)) - para.h_m_mV9(j,:) .* (1 ./ para.h_v_mV9(j,:)));
            
            %d * h matrix
            v_v_n9jk(j,k) = (1 ./ ((1 ./ para.v_v(j,k)) - (1 ./ para.h_v_v9(j,k))));
            m_v_n9jk(j,k) = v_v_n9jk(j,k) .* (para.m_v(j,k) .* (1 ./ para.v_v(j,k)) - para.h_m_v9(j,k) .* (1 ./ para.h_v_v9(j,k)));
